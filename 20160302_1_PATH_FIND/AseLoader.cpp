...
void AseLoader::Process_MESH_FACE_LIST(
	IN std::vector<D3DXVECTOR3>& pos, 
	OUT std::vector<FVF_PositionNormalTexture>& vertex)
{
	int level = 0;
	do 
	{
...
void AseLoader::Process_ROT_TRACK(OUT AseFrame* frame)
{
	...
	do 
	{
		...
		{		
			...
			s.quaternion.x = axis.x * sinf(angle*0.5f);
			s.quaternion.y = axis.y * sinf(angle*0.5f);
			s.quaternion.z = axis.z * sinf(angle*0.5f);
			s.quaternion.w = cosf(angle * 0.5f);

			if (!rotTrack.empty())
			{
				s.quaternion = 
					rotTrack.back().quaternion * s.quaternion;
			}

...
void ObjLoader::LoadObj_OnlyVertex(
	OUT std::vector<D3DXVECTOR3>& vertex, 
	char* fileName)
{
	char fullPath[512];
	strcpy_s(fullPath, OBJ_DIRECTORY);
	...
	if (fp != nullptr)
	{
		...
		while (!feof(fp))
		{
			...
			if (tempBuffer[0] == 'v' && tempBuffer[1] == ' ' )
			{
				float x, y, z;
				sscanf_s(tempBuffer, "%*s %f %f %f", &x, &y, &z);
				point.push_back(D3DXVECTOR3(x, z, y));
			}
			else if (tempBuffer[0] == 'f')
			{
				int index[3];
				sscanf_s(tempBuffer, "%*s %d/%*d/%*d %d/%*d/%*d %d/%*d/%*d",
					&index[0], &index[2], &index[1]);
				vertex.push_back(point[index[0] - 1]);
				vertex.push_back(point[index[1] - 1]);
				vertex.push_back(point[index[2] - 1]);
			}
		}
	}
}